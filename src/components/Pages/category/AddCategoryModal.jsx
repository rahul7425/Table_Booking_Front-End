...
const AddCategoryModal = ({ onClose, onAdd }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        name: formData.name,
        slug: formData.name.replace(/\s+/g, "-").toLowerCase(), // slug from name
        description: formData.description,
        subCategories: formData.subCategory.split(",").map((s) => s.trim()),
      };

      // Step 1: Create category
      const res = await axiosInstance.post("/categories/create", payload);
                  ...
                  <div className="col-span-8 sm:col-span-4 relative">
                    <div className="flex items-center">
                      ...
                      <input
                        ...
                        type="text"
                        name="parentCategory" // ✅ Add name to link with state
